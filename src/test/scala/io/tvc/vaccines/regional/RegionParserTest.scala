...
import io.tvc.vaccines.regional.RegionParser._
import io.tvc.vaccines.regional.XSLXParser._
...
class RegionParserTest extends AnyWordSpec with Matchers with ParallelTestExecution {
  ...
  "RegionParser" should {
    ...
    "parse 8th July format docs" in {
      val book = new XSSFWorkbook(getClass.getResourceAsStream("/8-July-2021.xlsx"))
      create(book).flatMap(regionalTotals.runA).map(_.statistics.keySet.size) shouldBe Right(42)
    }

    "parse 5th August format docs" in {
      val book = new XSSFWorkbook(getClass.getResourceAsStream("/5-August-2021.xlsx"))
      create(book).flatMap(regionalTotals.runA).map(_.statistics.keySet.size) shouldBe Right(42)
    }
  }
}
